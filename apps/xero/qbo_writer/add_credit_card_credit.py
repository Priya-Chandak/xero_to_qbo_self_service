    ...
    try:
        ...
        for transaction in spend_money:
            if transaction["TotalAmount"] >= 0:
                a11 = db["QBO_COA"].find(
                    {"FullyQualifiedName": transaction.get("BankAccountName").replace(":", "-"), 'job_id': job_id})
                for x3 in a11:
                        ...
                        for line_item in line_items:
                            ...
                            for c1 in range(0, len(QBO_COA)):
                                if transaction.get("BankAccountName").strip().lower() == QBO_COA[c1][
                                    ...
                                    break
                                elif transaction.get("BankAccountName").replace(":", "-") == QBO_COA[c1][
                                    'FullyQualifiedName']:
                                    QuerySet2["value"] = QBO_COA[c1]['Id']
                                    QuerySet2["name"] = QBO_COA[c1]['FullyQualifiedName']
                                    break
                        ...
                        if start_date1 is not None and end_date1 is not None:
                            if (spend_money_date1 >= start_date1) and (
                                ...
                                post_data_in_qbo(purchase_url, headers, payload, db["xero_receive_money"], _id, job_id,
                                                 task_id, transaction.get("BankTransactionID"))
                        else:
                            post_data_in_qbo(purchase_url, headers, payload, db["xero_receive_money"], _id, job_id,
                                             task_id, transaction.get("BankTransactionID"))
