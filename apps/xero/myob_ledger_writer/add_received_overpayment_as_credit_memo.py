                    ...
                    for b in range(0, len(chart_of_account)):
                        if xero_coa[a]["Name"].lower().strip() == chart_of_account[b]["Name"].lower().strip():
                            if chart_of_account[b]["Account_Type"] in ['Bank', 'BANK']:
                                ...
                                if 'Reference' in xero_receive_overpayment[i] and xero_receive_overpayment[i][
                                    'Reference'] != "":
                                    if len(xero_receive_overpayment[i] and xero_receive_overpayment[i][
                                        'Reference']) > 15:
                                ...
                                Queryset1['CustomerPurchaseOrderNumber'] = xero_receive_overpayment[i][
                                    'BankTransactionID']

                                for c1 in range(0, len(myob_customer)):
                                    if myob_customer[c1]["Company_Name"] == xero_receive_overpayment[i]['ContactName'][
                                                                            0:50]:
                                        Customer['UID'] = myob_customer[c1]["UID"]
                                    elif myob_customer[c1]["Company_Name"].startswith(
                                            xero_receive_overpayment[i]['ContactName']) and \
                                ...
                                for j in range(0, len(xero_receive_overpayment[i]['Line'])):
                                    ...
                                    if xero_receive_overpayment[i]['Line'][j]['LineAmount'] < 0:
                                        ...
                                        lineitem["UnitPrice"] = abs(
                                            xero_receive_overpayment[i]['Line'][j]['LineAmount'])
                                    ...
                                    for i2 in range(0, len(chart_of_account)):
                                        for j6 in range(0, len(xero_coa)):
                                            if xero_receive_overpayment[i]['Line'][j]['AccountCode'] == xero_coa[j6][
                                                "Code"]:
                                                if xero_coa[j6]["Name"] == chart_of_account[i2]["Name"]:
                                                    lineaccount['UID'] = chart_of_account[i2]['UID']
                                                    break

                                            elif xero_receive_overpayment[i]['Line'][j]['AccountCode'] == \
                                                    chart_of_account[i2]['DisplayId']:
                                                lineaccount['UID'] = chart_of_account[i2]['UID']
                                                break

                                    for n in range(0, len(xero_archived_coa)):
                                        for p1 in range(0, len(chart_of_account)):
                                            if xero_receive_overpayment[i]['Line'][j]['AccountCode'] == \
                                                    xero_archived_coa[n]['Code']:
                                    ...
                                    for j3 in range(0, len(taxcode_myob)):
                                        for j2 in range(0, len(xero_tax)):
                                            if 'TaxType' in xero_receive_overpayment[i]['Line'][j]:
                                                if xero_receive_overpayment[i]['Line'][j]["TaxType"] != None and \
                                                        xero_spend_overpayment[i]['Line'][j][
                                                            "TaxType"] != 'NONE':
                                                    if xero_receive_overpayment[i]['Line'][j]['TaxType'] == \
                                                            xero_tax[j2]['TaxType']:
                                                        ...
                                                        elif xero_tax[j2]['TaxType'] in ['EXEMPTCAPITAL',
                                                                                         'EXEMPTEXPENSES',
                                                                                         'EXEMPTEXPORT',
                                                                                         'EXEMPTOUTPUT']:
                                                        ...
                                                        elif xero_tax[j2]['TaxType'] in ["BASEXCLUDED", "BAS-W1",
                                                                                         "BAS-W2", "NONE"]:
                                                            if taxcode_myob[j3]['Code'] == 'N-T':
                                                                taxcode['UID'] = taxcode_myob[j3]['UID']

                                                elif xero_receive_overpayment[i]['Line'][j]['TaxType'] in ['NONE',
                                                                                                           None]:
                                                    if taxcode_myob[j3]['Code'] == 'N-T':
                                                        taxcode['UID'] = taxcode_myob[j3]['UID']
                                    ...
                                    if xero_receive_overpayment[i]['is_pushed'] == 0:
                                        asyncio.run(
                                            post_data_in_myob(url1, headers, payload,
                                                              dbname['xero_receive_overpayment'], _id, job_id, task_id,
                                                              id_or_name_value_for_error))
                                    else:
                                        pass
