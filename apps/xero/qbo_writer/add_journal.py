        ...
        for i in range(0, len(QuerySet1)):
            ...
            for j in range(0, len(QuerySet1[i]['Line'])):
                ...
                if QuerySet1[i]['Line'][j]['LineAmount'] >= 0:
                    if QuerySet1[i]['LineAmountTypes'] == 'Inclusive':
                        QuerySet12['NetAmountTaxable'] = round(
                            QuerySet1[i]['Line'][j]['LineAmount'] / (100 + taxrate1) * 100, 2)
                        QuerySet11['Amount'] = abs(QuerySet1[i]['Line'][j]['TaxAmount'])
                        QuerySet3['Amount'] = round(abs(QuerySet1[i]['Line'][j]['LineAmount'] / (100 + taxrate1) * 100),
                                                    2)
                    ...
                    if QuerySet1[i]['LineAmountTypes'] == 'Inclusive':
                        ...
                        QuerySet6['Amount'] = round(abs(QuerySet1[i]['Line'][j]['LineAmount'] / (100 + taxrate1) * 100),
                                                    2)
                        if 'Description' in QuerySet1[i]['Line'][j]:
            ...
            if (start_date != '' and end_date != ''):
                if (journal_date1 >= start_date1) and (journal_date1 <= end_date1):
                    post_data_in_qbo(url, headers, payload, journal1, _id, job_id, task_id,
                                     QuerySet1[i]["ManualJournalID"])
                else:
