                ...
                if len(QuerySet[i]['Line']) >= 1:
                    ...
                    for j in range(0, len(QuerySet[i]['Line'])):
                        ...
                        for j1 in range(0, len(QBO_COA)):
                            if QuerySet[i]['BankAccountName'].lower().strip() == QBO_COA[j1][
                        ...
                        if 'TaxType' in QuerySet[i]['Line'][j]:
                            for j4 in range(0, len(QBO_Tax)):
                                if QuerySet[i]['Line'][j]['TaxType'] == "BASEXCLUDED" or QuerySet[i]['Line'][j][
                                    'TaxType'] == None or QuerySet[i]['Line'][j]['TaxType'] == "NONE":
                                ...
                                elif QuerySet[i]['Line'][j]['TaxType'] in ['INPUTTAXED', 'EXEMPTCAPITAL',
                                                                           'EXEMPTEXPENSES', 'EXEMPTEXPORT']:
                                    if 'taxrate_name' in QBO_Tax[j4]:
                                ...
                                elif QuerySet[i]['Line'][j]['TaxType'] == "CAPEXINPUT" or QuerySet[i]['Line'][j][
                                    'TaxType'] == "INPUT":
                                    if 'taxrate_name' in QBO_Tax[j4]:
                                        if "GST (purchases)" in QBO_Tax[j4]['taxrate_name']:
                                            QuerySet8['value'] = QBO_Tax[j4]['taxcode_id']
                                ...
                                elif QuerySet[i]['Line'][j]['TaxType'] == "GSTONCAPIMPORTS" or QuerySet[i]['Line'][j][
                        ...
                        if QuerySet[i]['LineAmountTypes'] == 'Inclusive':
                            ...
                            QuerySet4['TaxInclusiveAmt'] = abs(
                                round((QuerySet[i]['Line'][j]['LineAmount']) * (100 + taxrate1) / 100, 2))
                            ...
                            for j31 in range(0, len(xero_coa1)):
                                if 'Code' in xero_coa1[j31]:
                                    if QuerySet[i]['Line'][j]['AccountCode'] == xero_coa1[j31]["Code"]:
                                        ...
                                        elif QBO_COA[j3]['Name'].startswith(xero_coa1[j31]["Name"]) and QBO_COA[j3][
                                            'Name'].endswith(xero_coa1[j31]["Code"]):
                                            QuerySet5['value'] = QBO_COA[j3]["Id"]
                                            QuerySet5['name'] = QBO_COA[j3]["Name"]
                        ...
                        for k1 in range(0, len(QBO_Supplier)):
                            if QBO_Supplier[k1]["DisplayName"].startswith(QuerySet[i]["ContactName"]) and \
                                    QBO_Supplier[k1]["DisplayName"].endswith(" - S"):
                    ...
                    if (start_date != '' and end_date != ''):
                        if (spend_money_date1 >= start_date1) and (spend_money_date1 <= end_date1):
                            post_data_in_qbo(purchase_url, headers, payload, db["xero_receive_money"], _id, job_id,
                                             task_id, QuerySet[i]["BankTransactionID"])

                        else:
                            print("No Spend Money Transaction Within these dates")
                    else:
                        post_data_in_qbo(purchase_url, headers, payload, db["xero_receive_money"], _id, job_id, task_id,
                                         QuerySet[i]["BankTransactionID"])
