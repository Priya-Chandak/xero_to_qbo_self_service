...
def add_xero_open_bill(job_id,task_id):
    try:
        ...
        final_bill1 = db['xero_open_bill'].find({'job_id':job_id})
                    ...
                    if final_bill[i]['TotalAmount'] >= 0:
                        if (len(final_bill[i]['Line']) >= 1):
                                    ...
                                    if 'ItemCode' in final_bill[i]['Line'][j]:
                                        ...
                                        for j3 in range(0, len(QBO_supplier)):
                                            if QBO_supplier[j3]["DisplayName"].startswith(final_bill[i]["ContactName"]) and QBO_supplier[j3]["DisplayName"].endswith(" - S"):
                                                ...
                                                QuerySet4["value"] = QBO_supplier[j3]["Id"]
                                                continue
                                            elif QBO_supplier[j3]["DisplayName"].startswith(final_bill[i]["ContactName"]) and QBO_supplier[j3]["DisplayName"].endswith("-S"):
                            ...
                            bill_date1 = datetime.strptime(
                                bill_date, '%Y-%m-%d')
                            post_data_in_qbo(url1, headers, payload, db["xero_open_bill"], _id, job_id, task_id, final_bill[i]['Inv_No'])
                            ...
                            for j in range(0, len(final_bill[i]['Line'])):
                                ...
                                post_data_in_qbo(url2, headers, payload, db["xero_open_bill"], _id, job_id, task_id, final_bill[i]['Inv_No'])
