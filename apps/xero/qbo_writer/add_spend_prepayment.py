        ...
        for i in range(0, len(QuerySet1)):
            ...
            if start_date1 is not None and end_date1 is not None:
                if (journal_date1 >= start_date1) and (journal_date1 <= end_date1):
                    post_data_in_qbo(url, headers, payload,db["xero_spend_prepayment"],_id, job_id,task_id,QuerySet1[i]["Reference"] )
            else:
                post_data_in_qbo(url, headers, payload,db["xero_spend_prepayment"],_id, job_id,task_id,QuerySet1[i]["Reference"] )
...
def add_spend_overpayment(job_id,task_id):
    try:
        ...
        for i in range(0, len(QuerySet1)):
            ...
            if start_date1 is not None and end_date1 is not None:
                if (journal_date1 >= start_date1) and (journal_date1 <= end_date1):
                    post_data_in_qbo(url, headers, payload,db["xero_spend_overpayment"],_id, job_id,task_id,QuerySet1[i]["Reference"] )
            else:
                post_data_in_qbo(url, headers, payload,db["xero_spend_overpayment"],_id, job_id,task_id,QuerySet1[i]["Reference"] )
