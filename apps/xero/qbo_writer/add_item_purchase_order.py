...
def add_xero_item_purchase_order(job_id):
    ...
    try:
        logging.info("Started executing xero -> qbowriter -> add_item_purchase_order -> add_xero_item_purchase_order")
        ...
        for i in range(0, len(xero_purchase_order)):
            ...
            for j in range(0, len(xero_purchase_order[i]["Line"])):
                ...
                for p6 in range(0, len(QBO_tax)):
                    if xero_purchase_order[i]["Line"][j]["taxcode"] == "BASEXCLUDED":
                ...
                if xero_purchase_order[i]["IsTaxInclusive"] == "Inclusive":
                    if xero_purchase_order[i]["Subtotal"] > 0:
                        ...
                        if (
                            ...
                            TaxDetail["Amount"] = 0
                            TaxLineDetail["NetAmountTaxable"] = xero_purchase_order[i][
                                "Line"
                            ][j]["Total"]
                        elif xero_purchase_order[i]["Line"][j]["taxcode"] == "N-T":
                            TaxDetail["Amount"] = 0
                    ...
                    if xero_purchase_order[i]["Subtotal"] > 0:
                        ...
                        ItemBasedExpenseLineDetail["UnitPrice"] = xero_purchase_order[
                            i
                        ]["Line"][j]["UnitPrice"]
                        salesitemline["Amount"] = round(
                            xero_purchase_order[i]["Line"][j]["UnitPrice"]
                ...
                if xero_purchase_order[i]["Line"][j]["DiscountPercent"] > 0:
