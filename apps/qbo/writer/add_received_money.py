                        ...
                        if len(QuerySet[i]['Line']) >= 1 and QuerySet[i]['Line'] != [{}]:
                            ...
                            for j in range(0, len(QuerySet[i]['Line'])):
                                ...
                                for j4 in range(0, len(QBO_tax)):
                                    ...
                                    elif QuerySet[i]['Line'][j]['tax_code'] == "N-T" or QuerySet[i]['Line'][j][
                                        'tax_code'] == None:
                                        if 'taxrate_name' in QBO_tax[j4]:
                                ...
                                for j1 in range(0, len(qbo_coa)):
                                    if QuerySet[i]['main_account'].lower().strip() == qbo_coa[j1][
                                        'Name'].lower().strip():
                                ...
                                for j3 in range(0, len(qbo_coa)):
                                    if (QuerySet[i]['Line'][j]['line_account'] + "-" + QuerySet[i]['Line'][j][
                                        'account_id']) == qbo_coa[j3]['Name']:
                                        QuerySet5['name'] = qbo_coa[j3]['Name']
                                        QuerySet5['value'] = qbo_coa[j3]['Id']

                                    elif QuerySet[i]['Line'][j]['line_account'].lower().strip() == qbo_coa[j3][
                                        'Name'].lower().strip():
                                        QuerySet5['name'] = qbo_coa[j3]['Name']
                                        QuerySet5['value'] = qbo_coa[j3]['Id']
                            ...
                            if QuerySet[i]['total_amount'] >= 0:
                                ...
                                if (start_date != '' and end_date != ''):
                                    if (receive_money_date1 >= start_date1) and (receive_money_date1 <= end_date1):
                                        post_data_in_qbo(deposit_url, headers, payload, received_money_data, _id,
                                                         job_id, task_id, QuerySet[i]['ref_no'])
                                    else:
                                        print("No Receive Money Transaction Within these dates")

                                else:
                                    post_data_in_qbo(deposit_url, headers, payload, received_money_data, _id, job_id,
