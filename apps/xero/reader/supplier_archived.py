...
def get_xero_archived_supplier(job_id, task_id):
    try:
        ...
        xero_archived_customer = dbname['xero_archived_customer']
        xero_archived_supplier = dbname['xero_archived_supplier']
        payload, base_url, headers = get_settings_xero(job_id)

        main_url = f"{base_url}/Contacts?includeArchived=True"
        ...
        for pages in range(1, no_of_pages + 1):
            url = f"{base_url}/Contacts/?includeArchived=True&page={pages}"
            print(url)
        ...
        if dbname['xero_archived_supplier'].count_documents({'job_id': job_id}) != 0:
            pass
        else:
            dbname['xero_archived_supplier'].insert_many(supplier)

        step_name = "Reading data from Archived supplier"
