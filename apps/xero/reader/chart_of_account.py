...
def get_coa(job_id,task_id):
    ...
    try:
        ...
        response1 = requests.request("GET", main_url, headers=headers, data=payload, timeout=5)
            ...
            if len(JsonResponse1)>0:
                ...
                for i in range(0, len(JsonResponse1)):
                    if JsonResponse1[i]["Status"] == "ACTIVE":
                        ...
                        QuerySet = {"job_id": job_id,"task_id":task_id,"is_pushed":0,"error":None, "AccountID": JsonResponse1[i]["AccountID"], "Name": JsonResponse1[i]["Name"],
                                    "payload":None,"Type": JsonResponse1[i]["Type"],"BankAccountType": JsonResponse1[i]["BankAccountType"], "TaxType": JsonResponse1[i]["TaxType"],
                        ...
                        if 'SystemAccount' in JsonResponse1[i]:
                            QuerySet['SystemAccount']= JsonResponse1[i]["SystemAccount"]
                        else:
                            QuerySet['SystemAccount']=None
