...
def add_negative_spend_money(job_id, task_id):
    try:
        ...
        for i in range(0, len(QuerySet)):
            ...
            if QuerySet[i]['Total_Amount'] < 0 and QuerySet[i]['UID'] in ['a571928a-34e8-4678-916a-1a34a2cd94e2',
                                                                          '8060821b-43af-43cc-8473-7ede5c68cb11']:
                print(i)
                            ...
                            if len(QuerySet[i]['Lines']) >= 1 and QuerySet[i]['Lines'] != [{}]:
                                ...
                                for j in range(0, len(QuerySet[i]['Lines'])):
                                    ...
                                    for j4 in range(0, len(QBO_tax)):
                                        ...
                                        elif (QuerySet[i]['Lines'][j]['taxcode'] == "FRE") or (
                                                QuerySet[i]['Lines'][j]['taxcode'] == "GNR"):
                                            if 'taxrate_name' in QBO_tax[j4]:
                                        ...
                                        elif QuerySet[i]['Lines'][j]['taxcode'] == "N-T" or QuerySet[i]['Lines'][j][
                                            'taxcode'] == None:
                                            if 'taxrate_name' in QBO_tax[j4]:
                                    ...
                                    for j1 in range(0, len(QBO_COA)):
                                        if QuerySet[i]['pay_from'].lower().strip() == QBO_COA[j1][
                                            'Name'].lower().strip():
                                    ...
                                    if (QuerySet[i]['contact_name'] != None and QuerySet[i][
                                ...
                                if (start_date != '' and end_date != ''):
                                    if (receive_money_date1 >= start_date1) and (receive_money_date1 <= end_date1):
                                        post_data_in_qbo(deposit_url, headers, payload, db['myob_spend_money'], _id,
                                                         job_id, task_id, QuerySet[i]['payment_no'])
                                    else:
                                        print("No Receive Money Transaction Within these dates")

                                else:
                                    post_data_in_qbo(deposit_url, headers, payload, db['myob_spend_money'], _id, job_id,
                                                     task_id, QuerySet[i]['payment_no'])
