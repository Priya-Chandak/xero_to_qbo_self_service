                            ...
                            for j in range(0, len(QuerySet[i]['Line'])):
                                if 'AccountCode' in QuerySet[i]['Line'][j]:
                                    ...
                                    for j4 in range(0, len(QBO_tax)):
                                        if QuerySet[i]['Line'][j]['TaxType'] == "NONE" or QuerySet[i]['Line'][j][
                                            'TaxType'] == "BASEXCLUDED" or QuerySet[i]['Line'][j]['TaxType'] == None:
                                        ...
                                        elif QuerySet[i]['Line'][j]['TaxType'] in ["EXEMPTCAPITAL", "EXEMPTEXPENSES",
                                                                                   "EXEMPTEXPORT", "EXEMPTOUTPUT",
                                                                                   "GSTONCAPIMPORTS", "GSTONIMPORTS",
                                                                                   "INPUTTAXED"]:
                                            if 'taxrate_name' in QBO_tax[j4]:
                                        ...
                                        elif QuerySet[i]['Line'][j]['TaxType'] in ["TAX001", "CAPEXINPUT", "INPUT",
                                                                                   "OUTPUT"]:
                                            if 'taxrate_name' in QBO_tax[j4]:
                                    ...
                                    for j1 in range(0, len(QBO_COA)):
                                        for j11 in range(0, len(xero_coa1)):
                                            if QuerySet[i]['BankAccountID'] == xero_coa1[j11]['AccountID']:
                                                if xero_coa1[j11]['Name'].lower().strip() == QBO_COA[j1][
                                                    'Name'].lower().strip():
                                                    QuerySet2['name'] = QBO_COA[j1]['Name']
                                                    QuerySet2['value'] = QBO_COA[j1]['Id']
                                                    break
                                                elif xero_coa1[j11]['Code'] not in ["", None]:
                                                    if QBO_COA[j1]['Name'].startswith(xero_coa1[j11]['Name']) and \
                                                            QBO_COA[j1]['Name'].endswith(xero_coa1[j11]['Code']):
                                                        QuerySet2['name'] = QBO_COA[j1]['Name']
                                                        QuerySet2['value'] = QBO_COA[j1]['Id']
                                                        break

                                            elif QuerySet[i]['BankAccountName'].replace(":", "-") == QBO_COA[j1][
                                                'Name']:
                                                QuerySet2['name'] = QBO_COA[j1]['Name']
                                                QuerySet2['value'] = QBO_COA[j1]['Id']
                                                break
                                            elif QuerySet[i]['BankAccountName'].lower().strip() == QBO_COA[j1][
                                                'Name'].lower().strip():
                                                QuerySet2['name'] = QBO_COA[j1]['Name']
                                                QuerySet2['value'] = QBO_COA[j1]['Id']
                                                break
                                    ...
                                    for j31 in range(0, len(QBO_Customer)):
                                        if QBO_Customer[j31]["FullyQualifiedName"].startswith(
                                                QuerySet[i]["ContactName"]) and QBO_Customer[j31][
                                            "DisplayName"].endswith(" - C"):
                            ...
                            if QuerySet[i]['TotalAmount'] >= 0:
                                ...
                                if (start_date != '' and end_date != ''):
                                    if (receive_money_date1 >= start_date1) and (receive_money_date1 <= end_date1):
                                        post_data_in_qbo(deposit_url, headers, payload, xero_receive_money1, _id,
                                                         job_id, task_id, QuerySet[i]['BankTransactionID'])

                                    else:
                                        print("No Receive Money Transaction Within these dates")

                                else:
                                    post_data_in_qbo(deposit_url, headers, payload, xero_receive_money1, _id, job_id,
                                                     task_id, QuerySet[i]['BankTransactionID'])
