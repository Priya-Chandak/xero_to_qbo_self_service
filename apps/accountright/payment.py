...
def get_accountright_payment(job_id):
    try:
        db = get_mongodb_database()
        Collection = db["accountright_payment"]

        no_of_records = db["accountright_payment"].count_documents({})
        ...
        if JsonResponse["NextPageLink"] is not None:
            get_accountright_payment(job_id)
