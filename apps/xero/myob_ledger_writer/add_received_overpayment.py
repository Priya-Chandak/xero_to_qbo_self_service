...
def add_receive_overpayment_as_received_money(job_id, task_id):
    try:
        ...
        for i in range(0, len(xero_receive_overpayment)):
            ...
            for i1 in range(0, len(xero_coa)):
                for i2 in range(0, len(myob_coa)):
                    if xero_receive_overpayment[i]['BankAccountName'] == xero_coa[i1]['Name']:
            ...
            for i3 in range(0, len(myob_supplier)):
                ...
                elif myob_supplier[i3]['FirstName'].startswith(xero_receive_overpayment[i]['ContactName']) and \
            ...
            for j in range(0, len(xero_receive_overpayment[i]['Line'])):
                ...
                taxcode = {}
                tracking = {}
                lineitem['Amount'] = xero_receive_overpayment[i]['Line'][j]['LineAmount']
                lineitem['Memo'] = xero_receive_overpayment[i]['Line'][j]['Description']
                lineitem['UnitCount'] = xero_receive_overpayment[i]['Line'][j]['Quantity']
                Queryset1['Memo'] = xero_receive_overpayment[i]['Line'][j]['Description']

                for i2 in range(0, len(myob_coa)):
                    for i1 in range(0, len(xero_coa)):
                        if xero_receive_overpayment[i]['Line'][j]['AccountCode'] == xero_coa[i1]['Code']:
                            if xero_coa[i1]['Name'] == myob_coa[i2]['Name']:
                ...
                for i21 in range(0, len(myob_job)):
                    if 'TrackingName' in xero_receive_overpayment[i]['Line'][j]:
                        if xero_receive_overpayment[i]['Line'][j]['TrackingName'] == myob_job[i21]['Name']:
                ...
                for j3 in range(0, len(taxcode_myob)):
                    for j2 in range(0, len(xero_tax)):
                        if xero_receive_overpayment[i]['Line'][j]['TaxType'] == xero_tax[j2]['TaxType']:
                            ...
                            if xero_receive_overpayment[i]['Line'][j]['TaxType'] in ["BASEXCLUDED", "BAS-W1", "BAS-W2",
                                                                                     "NONE"]:
                                if taxcode_myob[j3]['Code'] == 'N-T':
                                    taxcode['UID'] = taxcode_myob[j3]['UID']
                                    taxrate1 = taxcode_myob[j3]['Rate']
                ...
                if xero_receive_overpayment[i]['LineAmountTypes'] == 'Inclusive':
            ...
            id_or_name_value_for_error = (
                xero_receive_overpayment[i]['Reference']
                if xero_receive_overpayment[i]['Reference'] is not None
                else "")

            payload1, base_url, headers = get_settings_myob(job_id)
            url = f"{base_url}/Banking/ReceiveMoneyTxn"
            asyncio.run(
                post_data_in_myob(url, headers, payload, dbname['xero_receive_overpayment'], _id, job_id, task_id,
                                  id_or_name_value_for_error))
