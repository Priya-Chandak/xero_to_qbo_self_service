        ...
        for i in range(0, len(bill_credit_memo_refund)):
            ...
            for k1 in range(0, len(myob_supplier)):
                ...
                if myob_supplier[k1]["CompanyName"] == bill_credit_memo_refund[i]["Contact"][0:50]:
                    ...
                    if myob_supplier[k1]["CompanyName"].startswith(bill_credit_memo_refund[i]["Contact"]) and myob_supplier[k1]["CompanyName"].endswith("- S"):
                        Supplier['UID'] = myob_supplier[k1]["UID"]
            ...
            if (bill_credit_memo_refund[i]["Status"] != 'SUBMITTED') and (bill_credit_memo_refund[i]['Status'] != "DRAFT") and (
                    bill_credit_memo_refund[i]['Status'] != "VOIDED") and (bill_credit_memo_refund[i]['Status'] != "DELETED"):
                ...
                asyncio.run(
                    post_data_in_myob(url1, headers, payload, dbname['xero_supplier_credit_cash_refund'], _id, job_id, task_id,
                                    id_or_name_value_for_error))
