        ...
        for i in range(0, len(Item)):
                        ...
                        if 'UnitPrice' in Item[i]["SalesDetails"][0]:
                            ...
                            for j7 in range(0,len(xero_coa)):
                                if "AccountCode" in Item[i]["SalesDetails"][0]:
                                    if Item[i]["SalesDetails"][0]["AccountCode"] == str(chart_of_account[j]["DisplayId"]): 
            ...
            if 'PurchaseDetails' in Item[i]:
                for j1 in range(0, len(chart_of_account)):
                    for j8 in range(0,len(xero_coa)):
                        if "AccountCode" in Item[i]["PurchaseDetails"][0] or 'COGSAccountCode' in Item[i]["PurchaseDetails"][0]:
                            if Item[i]["PurchaseDetails"] != None and Item[i]["PurchaseDetails"] != [{}]:

                                if 'COGSAccountCode' in Item[i]["PurchaseDetails"][0]:
                                    if Item[i]["PurchaseDetails"][0]["COGSAccountCode"] == str(chart_of_account[j1]["Number"]):
                                    ...
                                    if Item[i]["PurchaseDetails"][0]["AccountCode"] == str(chart_of_account[j1]["Number"]):
                                        expenseacc["UID"] = chart_of_account[j1]["UID"]
                                        QuerySet3 = expenseacc
                                        break
            ...
            for j3 in range(0, len(taxcode_myob)):
                for j02 in range(0, len(xero_tax)):
                    if 'SalesDetails' in Item[i]:
                        if 'TaxType' in Item[i]["SalesDetails"][0] and 'TaxType' in Item[i]["SalesDetails"][0]['TaxType']!="":
                            if Item[i]["SalesDetails"][0]["TaxType"] == xero_tax[j02]['TaxType']:
                                ...
                                elif xero_tax[j02]['TaxType'] in ["EXEMPTCAPITAL", 'EXEMPTEXPENSES', 'EXEMPTOUTPUT', 'EXEMPTEXPORT']:
                    ...
                    if 'PurchaseDetails' in Item[i]:
                        ...
                        if 'TaxType' in Item[i]["PurchaseDetails"][0] and Item[i]["PurchaseDetails"][0]["TaxType"]!="":
