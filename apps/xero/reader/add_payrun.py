...
def add_xero_payrun(job_id, task_id):
    try:
        ...
        xero_payrun_setting1 = dbname["xero_payrun_setting"].find(
            {"job_id": job_id})
        xero_payrun_setting = []
        ...
        for i in range(0, len(QuerySet1)):
            ...
            if QuerySet1[i]['payrun']['PayRunStatus'] == 'POSTED':
                ...
                if float(QuerySet1[i]['payrun']['Wages']) != 0:
                    ...
                    QuerySet3["Amount"] = abs(
                        float(QuerySet1[i]['payrun']['Wages']))
                ...
                if float(QuerySet1[i]['payrun']['Super']) != 0:
                    ...
                    JournalEntryLineDetail["PostingType"] = "Debit"
                    QuerySet3["Amount"] = abs(
                        float(QuerySet1[i]['payrun']['Super']))
                ...
                if float(QuerySet1[i]['payrun']['NetPay']) != 0:
                    ...
                    QuerySet3["Amount"] = abs(
                        float(QuerySet1[i]['payrun']['NetPay']))
            ...
            post_data_in_qbo(
                url, headers, payload, dbname['xero_payrun'], _id, job_id, task_id, QuerySet1[i]['payrun'])

    except Exception as ex:
        logger.error(
            "Error in xero -> qbowriter -> add_xero_invoice_payment", ex)
