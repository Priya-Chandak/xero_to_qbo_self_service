    ...
    try:
        ...
        if response1.status_code == 200:
            ...
            if no_of_records > 0:
                ...
                step_name = "Reading data from xero bill"
...
def get_bill_suppliers(job_id, task_id):
    ...
    try:
        start_date, end_date = get_job_details(job_id)
        dbname = get_mongodb_database()
        xero_bill = dbname['xero_bill_suppliers']
        payload, base_url, headers = get_settings_xero(job_id)
        ...
        if (start_date == '' and end_date == ''):
            ...
            m2 = int(end_date[5:7])
            d2 = int(end_date[8:])
            main_url = f"{base_url}/Invoices?where=Date%3E%3DDateTime({y1}%2C{m1}%2C{d1})%20AND%20Date%3C%3DDateTime({y2}%2C{m2}%2C{d2})"
            ...
            if no_of_records > 0:
                ...
                for pages in range(1, no_of_pages + 1):
                    if (start_date == '' and end_date == ''):
                        ...
                        url = f"{base_url}/Invoices?page={pages}&where=Date%3E%3DDateTime({y1}%2C{m1}%2C{d1})%20AND%20Date%3C%3DDateTime({y2}%2C{m2}%2C{d2})"
