    ...
    try:
        ...
        for i in range(0, len(QuerySet)):
            if QuerySet[i]['TotalAmount'] < 0:
                a11 = db["QBO_COA"].find(
                    {"FullyQualifiedName": QuerySet[i]["BankAccountName"].strip(), 'job_id': job_id})
                for x3 in a11:
                        ...
                        if len(QuerySet[i]['Line']) >= 1 and QuerySet[i]['Line'] != [{}]:
                            ...
                            for j in range(0, len(QuerySet[i]['Line'])):
                                ...
                                for j4 in range(0, len(QBO_tax)):
                                    ...
                                    elif QuerySet[i]['Line'][j]['TaxType'] in ["OUTPUT", 'CAPEXINPUT', 'INPUT']:
                                        if 'taxrate_name' in QBO_tax[j4]:
                                    ...
                                    elif QuerySet[i]['Line'][j]['TaxType'] in ['EXEMPTOUTPUT', 'INPUTTAXED',
                                                                               'EXEMPTCAPITAL', 'EXEMPTEXPENSES',
                                                                               'EXEMPTEXPORT', 'GSTONCAPIMPORTS',
                                                                               'GSTONIMPORTS']:
                                        if 'taxrate_name' in QBO_tax[j4]:
                                ...
                                a1 = db["QBO_COA"].find(
                                    {"FullyQualifiedName": QuerySet[i]['BankAccountName'], 'job_id': job_id})
                                for x in a1:
                                ...
                                for j31 in range(0, len(QBO_Customer)):
                                    if QBO_Customer[j31]["DisplayName"].startswith(QuerySet[i]["ContactName"]) and \
                                            QBO_Customer[j31]["DisplayName"].endswith(" - C"):
                                        QuerySet7["value"] = QBO_Customer[j31]['Id']
                                        QuerySet7['name'] = QBO_Customer[j31]['DisplayName']
                                        QuerySet7['type'] = 'CUSTOMER'
                                    elif QuerySet[i]["ContactName"].lower().strip() == QBO_Customer[j31][
                                ...
                                for j3 in range(0, len(QBO_Supplier)):
                                    if QBO_Supplier[j3]["DisplayName"].startswith(QuerySet[i]["ContactName"]) and \
                                    ...
                                    for j31 in range(0, len(xero_coa1)):
                                        if 'Code' in xero_coa1[j31]:
                                            if QuerySet[i]['Line'][j]['AccountCode'] == xero_coa1[j31]["Code"]:
                                                if xero_coa1[j31]["Name"].lower().strip() == QBO_COA[j3][
                                                    'Name'].lower().strip():
                                                    QuerySet5['value'] = QBO_COA[j3]["Id"]
                                                    QuerySet5['name'] = QBO_COA[j3]["Name"]
                                                elif QBO_COA[j3]['Name'].startswith(xero_coa1[j31]["Name"]) and \
                                                        QBO_COA[j3]['Name'].endswith(xero_coa1[j31]["Code"]):
                                                    QuerySet5['value'] = QBO_COA[j3]["Id"]
                                                    QuerySet5['name'] = QBO_COA[j3]["Name"]
                                ...
                                if QuerySet[i]['Reference'] != None and QuerySet[i]['Reference'] != "":
                                    ...
                                    QuerySet4['CheckNum'] = QuerySet[i]['Type'] + "-" + QuerySet[i][
                                                                                            'BankTransactionID'][-6:]
                            ...
                            id_or_name_value_for_error = (
                                    str(QuerySet[i]["Date"]) + "-" + str(QuerySet[i]["TotalAmount"]) + "-" + str(
                                QuerySet[i]["ContactName"])
                            )
                            ...
                            if (start_date1 != '' and end_date1 != ''):
                                if (receive_money_date1 >= start_date1) and (receive_money_date1 <= end_date1):
                                    post_data_in_qbo(deposit_url, headers, payload, xero_spend_money1, _id, job_id,
                                                     task_id, id_or_name_value_for_error)

                                else:
                                    print("No Receive Money Transaction Within these dates")

                            else:
                                post_data_in_qbo(deposit_url, headers, payload, xero_spend_money1, _id, job_id, task_id,
                                                 id_or_name_value_for_error)

    except Exception as ex:
        logger.error(
            "Error in xero -> qbowriter -> add_negative_spend_money -> add_xero_negative_spend_money_as_receive_money",
            ex)
    ...
    try:
        ...
        for transaction in spend_money:
            if transaction["TotalAmount"] < 0:
                a11 = db["QBO_COA"].find(
                    {"FullyQualifiedName": transaction.get("BankAccountName").strip(), 'job_id': job_id})
                for x3 in a11:
                    if x3.get("AccountType") == "Credit Card":
                        ...
                        for line_item in line_items:
                            ...
                            taxrate1 = 0
                            if line_item.get("TaxType") == "BASEXCLUDED" or line_item.get(
                                    "TaxType") == None or line_item.get("TaxType") == "NONE":
                            ...
                            s1 = db["QBO_Supplier"].find(
                                {"DisplayName": transaction.get("ContactName"), 'job_id': job_id})
                            # s2 = db["QBO_Supplier"].find({"DisplayName": transaction.get("ContactName")})
                            s2 = db["QBO_Supplier"].find(
                                {"DisplayName": {'$regex': '- S$', '$regex': f'^{transaction.get("ContactName")}'},
                                 'job_id': job_id})
                        ...
                        print(payload)
                        id_or_name_value_for_error = (
                                str(transaction.get("Date")) + "-" + str(transaction.get("TotalAmount")) + "-" + str(
                            transaction.get("ContactName"))
                        )
                        ...
                        if start_date1 is not None and end_date1 is not None:
                            if (spend_money_date1 >= start_date1) and (
                                ...
                                post_data_in_qbo(purchase_url, headers, payload, db["xero_spend_money"], _id, job_id,
                                                 task_id, id_or_name_value_for_error)
                        else:
                            post_data_in_qbo(purchase_url, headers, payload, db["xero_spend_money"], _id, job_id,
                                             task_id, id_or_name_value_for_error)
