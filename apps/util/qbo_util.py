...
def retry_payload_for_excel_to_reckon(job_id, payload, _id, task_id, task_function_name):
    ...
    if task_function_name in function_name:
        ...
        status = asyncio.run(post_data_in_reckon(url, post_headers, payload, tablename, _id, job_id, task_id,
                                                 id_or_name_value_for_error))
        if status == 'success':
            return 'success'
        else:
...
def retry_payload_for_myob_to_qbo(job_id, payload, _id, task_id, task_function_name):
    ...
    base_url, headers, company_id, minorversion, get_data_header, report_headers = get_settings_qbo(job_id)
    static_function = {
        "Chart of account": [f"{base_url}/account?minorversion={minorversion}", dbname['classified_coa']],
        "Customer": [f"{base_url}/customer?minorversion={minorversion}", dbname['customer']],
        "Archieved Customer": [f"{base_url}/customer?minorversion={minorversion}",
                               dbname['archived_customer_in_invoice']],
    ...
    if task_function_name in function_name:
        ...
        status = post_data_in_qbo(base_url, headers, payload, dbname["xero_classified_coa"], _id, job_id, task_id,
                                  id_or_name_value_for_error)
