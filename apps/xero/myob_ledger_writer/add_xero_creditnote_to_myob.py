...
def add_xero_creditnote_to_myob(job_id, task_id):
    try:
        dbname = get_mongodb_database()
        # payload, base_url, headers = get_settings_myob(job_id)
        payload = {}
        xero_invoice = dbname['xero_creditnote'].find({"job_id": job_id})
        ...
        for i in range(0, len(multiple_invoice)):
            ...
            # if multiple_invoice[i]['Inv_No'] in ['CN10895']:
            ...
            if (multiple_invoice[i]["Status"] != 'SUBMITTED') and (multiple_invoice[i]['Status'] != "DRAFT") and (
                ...
                id_or_name_value_for_error = (
                    ...
                    else multiple_invoice[i]['TxnDate'])
