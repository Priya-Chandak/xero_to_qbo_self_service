...
def get_xero_bill_credit_memo_refund_payment(job_id, task_id):
    log_config1=log_config(job_id)
    try:
        start_date, end_date = get_job_details(job_id)
        dbname = get_mongodb_database()
        xero_bill_credit_memo_refund_payment = dbname["xero_bill_credit_memo_refund_payment"]
        ...
        if response1.status_code == 200:
            ...
            if len(r2) > 0:
                ...
                bill_payment = []
                bill_credit_memo_refund_payment = []
                creditnote_payment = []
                billcredit_payment = []
                ...
                for pages in range(1, no_of_pages + 1):
                    ...
                    for i in range(0, len(JsonResponse1)):
                        if (
                            ...
                            if JsonResponse1[i]["PaymentType"] == "APCREDITPAYMENT":
                                QuerySet["table_name"] = "xero_bill_credit_memo_refund_payment"
                                bill_credit_memo_refund_payment.append(QuerySet)
                ...
                if len(bill_credit_memo_refund_payment) > 0:
                    xero_bill_credit_memo_refund_payment.insert_many(bill_credit_memo_refund_payment)
