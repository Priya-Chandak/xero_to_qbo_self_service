...
def add_xero_archived_supplier_to_myobledger(job_id, task_id):
    try:
        ...
        # xero_supplier1 = dbname["xero_archived_supplier_in_bill"]

        xero_supplier1 = dbname['xero_archived_supplier']
        ...
        for i in range(0, len(data)):
            ...
            if "BankAccName" in data[i]:
                PaymentDetails["BankAccountName"] = data[i]["BankAccName"][0:32].upper()
            ...
            for j in range(0, len(data[i]['Address'])):
                ...
                if ('AddressLine2' in data[i]['Address'][j]) and ('AddressLine3' in data[i]['Address'][j]):
                    Queryset2['Street'] = data[i]['Address'][j]['AddressLine1'] + "," + data[i]['Address'][j][
                        'AddressLine2'] + "," + data[i]['Address'][j]['AddressLine3']
                elif ('AddressLine2' in data[i]['Address'][j]) and ('AddressLine3' not in data[i]['Address'][j]):
                    Queryset2['Street'] = data[i]['Address'][j]['AddressLine1'] + "," + data[i]['Address'][j][
                        'AddressLine2']
