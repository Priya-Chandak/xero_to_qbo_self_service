        ...
        for transaction in spend_money:
            if transaction["TotalAmount"] >= 0:
                ...
                for line_item in line_items:
                    ...
                    # a2=db["QBO_COA"].find({"FullyQualifiedName": transaction.get("BankAccountName").replace(":","-"),'job_id':job_id})
                    # if x in a1:
                    ...
                    for c1 in range(0, len(QBO_COA)):
                        if transaction.get("BankAccountName").strip().lower() == QBO_COA[c1][
                            'FullyQualifiedName'].strip().lower():
                    ...
                    taxrate1 = 0
                    if line_item.get("TaxType") == "BASEXCLUDED" or line_item.get("TaxType") == None or line_item.get(
                            "TaxType") == "NONE":
                        ...
                        for x in t1:
                          ...
                          or line_item.get("TaxType") == "TAX001"):
                    ...
                    c2 = db["QBO_Customer"].find(
                        {"DisplayName": {'$regex': '- C$', '$regex': f'^{transaction.get("ContactName")}'},
                         'job_id': job_id})
                    ...
                    for x3 in c1:
                        QuerySet93["value"] = x3.get("Id")
                        QuerySet93["name"] = x3.get("DisplayName")
                        break

                    for x4 in c2:
                        QuerySet93["value"] = x4.get("Id")
                        QuerySet93["name"] = x4.get("DisplayName")
                        break

                    QuerySet1["EntityRef"] = QuerySet91 if QuerySet91 != {} else QuerySet93
                ...
                if start_date1 is not None and end_date1 is not None:
                    if (spend_money_date1 >= start_date1) and (
                        ...
                        post_data_in_qbo(purchase_url, headers, payload, db["xero_spend_money"], _id, job_id, task_id,
                                         id_or_name_value_for_error)
                else:
                    post_data_in_qbo(purchase_url, headers, payload, db["xero_spend_money"], _id, job_id, task_id,
                                     id_or_name_value_for_error)
