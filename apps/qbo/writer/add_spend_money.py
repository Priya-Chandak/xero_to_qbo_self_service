            ...
            if QuerySet[i]['Total_Amount']>=0:
                ...
                for j in range(0, len(QuerySet[i]['Lines'])):
                    ...
                    if QuerySet[i]['IsTaxInclusive'] == True:
                        ...
                        QuerySet4['TaxInclusiveAmt'] = round(QuerySet[i]['Lines'][j]['line_amount']/(100+taxrate1)*100, 2)
                ...
                if (start_date != '' and end_date != ''):
                    if (spend_money_date1 >= start_date1) and (spend_money_date1 <= end_date1):
                        post_data_in_qbo(purchase_url, headers, payload,spend_money_data,_id,job_id,task_id, QuerySet[i]['payment_no'])
                    else:
