...
def add_open_receive_overpayment(job_id,task_id):
    try:
        logger.info("Started executing xero -> qbowriter -> add_receive_open_overpayment -> add_receive_overpayment")
        ...
        overpayment1 = db["xero_open_receive_overpayment"].find({"job_id":job_id})
        ...
        for i in range(0, len(QuerySet1)):
            ...
            for j in range(0, len(QuerySet1[i]["Line"])):
                ...
                for j4 in range(0, len(QBO_tax)):
                    if "TaxType" in QuerySet1[i]["Line"][j]:
                        if QuerySet1[i]["Line"][j]["TaxType"] == "BASEXCLUDED":
                            if QuerySet1[i]["Line"][j]["LineAmount"] >= 0:
                                if "taxrate_name" in QBO_tax[j4]:
                                    if "NOTAXP" == QBO_tax[j4]["taxrate_name"]:
                                ...
                                if "taxrate_name" in QBO_tax[j4]:
                                    if "taxrate_name" in QBO_tax[j4]:
                                        if "NOTAXP" == QBO_tax[j4]["taxrate_name"]:
                                            QuerySet13["value"] = QBO_tax[j4][
                                                "taxrate_id"
                                            ]
                                            QuerySet15["value"] = QBO_tax[j4][
                                                "taxcode_id"
                                            ]
                                            taxrate = QBO_tax[j4]["Rate"]
                                            taxrate1 = taxrate
                                            QuerySet12["TaxPercent"] = QBO_tax[j4][
                                                "Rate"
                                            ]
                        ...
                        elif QuerySet1[i]["Line"][j]["TaxType"] == "NONE":
                            if "taxrate_name" in QBO_tax[j4]:
                ...
                if QuerySet1[i]["Line"][j]["LineAmount"] >= 0:
                    ...
                    Q11["JournalEntryLineDetail"]["PostingType"] = "Credit"

                    for p5 in range(0, len(QBO_coa)):
                        if QBO_coa[p5]["AccountType"] == "Accounts Receivable":
            ...
            post_data_in_qbo(url, headers, payload,db["xero_open_receive_overpayment"],_id, job_id,task_id,QuerySet1[i]["Reference"] )
...
def add_open_spend_overpayment(job_id,task_id):
    try:
        logger.info("Started executing xero -> qbowriter -> xero_open_spend_overpayment -> add_spend_overpayment")
        ...
        overpayment1 = db["xero_open_spend_overpayment"].find({"job_id":job_id})
        ...
        for i in range(0, len(QuerySet1)):
            ...
            QuerySet2 = {"Line": []}
            QuerySet10 = {"TaxLine": []}
            QuerySet2["DocNumber"] = "SMOP-"+str(QuerySet1[i]["Reference"])
            QuerySet2["TxnDate"] = QuerySet1[i]["Date"][0:11]
            ...
            for j in range(0, len(QuerySet1[i]["Line"])):
                ...
                if QuerySet1[i]["Line"][j]["LineAmount"] >= 0:
                    ...
                    for v1 in range(0, len(QBO_supplier)):
                        ...
                        elif QBO_supplier[v1]["DisplayName"].startswith(QuerySet1[i]["ContactName"]) and (QBO_supplier[v1]["DisplayName"]).endswith("- S"):
                            EntityRef["value"] = QBO_supplier[v1]["Id"]
                            EntityRef["name"] = QBO_supplier[v1]["DisplayName"]
                            Entity["Type"] = "Vendor"
                            continue
