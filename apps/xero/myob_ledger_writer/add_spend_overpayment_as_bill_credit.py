                    ...
                    for b in range(0, len(chart_of_account)):
                        if xero_coa[a]["Name"].lower().strip() == chart_of_account[b]["Name"].lower().strip():
                            if chart_of_account[b]["Account_Type"] in ['Bank','BANK']:
                                ...
                                if 'Reference' in xero_spend_overpayment[i] and xero_spend_overpayment[i]['Reference'] != "":
                                ...
                                for k1 in range(0, len(myob_supplier)):
                                    ...
                                    if myob_supplier[k1]["CompanyName"] == xero_spend_overpayment[i]["ContactName"][0:50]:
                                        Supplier['UID'] = myob_supplier[k1]["UID"]
                                        break
                                    elif 'CompanyName' in myob_supplier[k1] and myob_supplier[k1]["CompanyName"]!= None:
                                        if myob_supplier[k1]["CompanyName"].startswith(xero_spend_overpayment[i]["ContactName"]) and myob_supplier[k1]["CompanyName"].endswith("- S"):
                                            Supplier['UID'] = myob_supplier[k1]["UID"]
                                            break
                                ...
                                for j in range(0, len(xero_spend_overpayment[i]["Line"])):
                                    ...
                                    for j3 in range(0, len(myob_job)):
                                        if 'TrackingID' in xero_spend_overpayment[i]["Line"][j]:
                                            if myob_job[j3]['Name'] == xero_spend_overpayment[i]["Line"][j]["TrackingID"]:
                                    ...
                                    if 'AccountCode' in xero_spend_overpayment[i]['Line'][j]:
                                        ...
                                        if 'AccountCode' in xero_spend_overpayment[i]["Line"][j]: 
                                            for j5 in range(0, len(chart_of_account)):
                                                for j51 in range(0, len(xero_coa)):
                                                    if xero_spend_overpayment[i]["Line"][j]["AccountCode"] == xero_coa[j51]['Code']:
                                            ...
                                            for j3 in range(0, len(taxcode_myob)):
                                                for k1 in range(0, len(xero_tax)):
                                                    ...
                                                    if 'TaxType' in xero_spend_overpayment[i]["Line"][j]:
                                                        if xero_spend_overpayment[i]["Line"][j]["TaxType"] != None and xero_spend_overpayment[i]["Line"][j][
                                                            "TaxType"] != 'NONE':
                                                            if xero_spend_overpayment[i]["Line"][j]["TaxType"] == xero_tax[k1]['TaxType']:
                                        ...
                                        if "Description" in xero_spend_overpayment[i]["Line"][j]:
                                            QuerySet3["Description"] = xero_spend_overpayment[i]["Line"][j]["Description"]
                                        
                                        if 'Quantity' in xero_spend_overpayment[i]["Line"][j]:
                                            if xero_spend_overpayment[i]["Line"][j]["LineAmount"] < 0:
                                                QuerySet3["ReceivedQuantity"] = -xero_spend_overpayment[i]["Line"][j]["Quantity"]
                                                QuerySet3["BillQuantity"] = -xero_spend_overpayment[i]["Line"][j]["Quantity"]
                                                QuerySet3["UnitCount"] = -xero_spend_overpayment[i]["Line"][j]["Quantity"]
                                                QuerySet3["UnitPrice"] = abs(xero_spend_overpayment[i]["Line"][j]["UnitAmount"])
                                                QuerySet3["Total"] = QuerySet3["UnitPrice"]*QuerySet3["UnitCount"]
                                            else:
                                                QuerySet3["ReceivedQuantity"] = xero_spend_overpayment[i]["Line"][j]["Quantity"]
                                                QuerySet3["BillQuantity"] = xero_spend_overpayment[i]["Line"][j]["Quantity"]
                                                QuerySet3["UnitCount"] = xero_spend_overpayment[i]["Line"][j]["Quantity"]
                                                QuerySet3["UnitPrice"] = xero_spend_overpayment[i]["Line"][j]["UnitAmount"]
                                                QuerySet3["Total"] = QuerySet3["UnitPrice"]*QuerySet3["UnitCount"]
                                        else:
                                            if xero_spend_overpayment[i]["Line"][j]["LineAmount"] < 0:
