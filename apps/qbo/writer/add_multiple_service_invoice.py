    ...
    try:
        ...
        for i in range(0, len(multiple_invoice)):
            ...
            for j in range(len(multiple_invoice[i]['account'])):
                ...
                if (multiple_invoice[i]['account'][j]['unit_count'] == 0) or (
                        multiple_invoice[i]['account'][j]['unit_count'] == None):
                ...
                if "DisplayID" in multiple_invoice[i]['account'][j]:
                    ...
                    for p4 in range(0, len(QBO_class)):
                        for j4 in range(0, len(Myob_Job1)):
                            if ('job' in multiple_invoice[i]['account'][j]) and (
                                    multiple_invoice[i]['account'][j]['job'] != None):
                                if multiple_invoice[i]['account'][j]['job']['Name'] == Myob_Job1[j4]['Name']:
                                    if (QBO_class[p4]['FullyQualifiedName'].startswith(Myob_Job1[j4]['Name'])) and (
                                            QBO_class[p4]['FullyQualifiedName'].endswith(Myob_Job1[j4]['Number'])):
                    ...
                    for p6 in range(0, len(QBO_tax)):
                        if 'taxcode' in multiple_invoice[i]['account'][j]:
                            if (multiple_invoice[i]['account'][j]['taxcode'] == "GST") or (
                                    multiple_invoice[i]['account'][j]['taxcode'] == "GCA"):
                ...
                if multiple_invoice[i]['TotalAmount'] >= 0:
                    ...
                    if (start_date != '' and end_date != ''):
                        ...
                        if (inv_date1 >= start_date1) and (inv_date1 <= end_date1):
                            ...
                            post_data_in_qbo(url1, headers, payload, db['service_invoice'], _id, job_id, task_id,
                    ...
                    if (start_date != '' and end_date != ''):
                        if (inv_date1 >= start_date1) and (inv_date1 <= end_date1):
                            ...
                            post_data_in_qbo(url2, headers, payload, db['service_invoice'], _id, job_id, task_id,
                                             multiple_invoice[i]['invoice_no'])
                            ...
                            if multiple_invoice[i]['IsTaxInclusive'] == False:
                                ...
                                if multiple_invoice[i]['Subtotal'] >= 0:
                                    if 'unit_count' not in multiple_invoice[i]['account'][j]:
                                        ...
                                        SalesItemLineDetail['Qty'] = multiple_invoice[i][
                                            'account'][j]['unit_count']
                                    SalesItemLineDetail['UnitPrice'] = multiple_invoice[i]['account'][j][
                                                                           'unit_price'] / (100 + taxrate1) * 100
